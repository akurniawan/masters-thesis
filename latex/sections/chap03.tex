...
% where the algorithm collects the set of words and their frequency
...
We define $w_n$ as the weights for each n-grams scores ($p_n$) where $\sum^{\infty}_{i=1} w_i = 1$. $w_n$ is usually set depending on the number of n-grams used in the calculation. When 4-grams is used, it is set to $\frac{1}{4}$.
