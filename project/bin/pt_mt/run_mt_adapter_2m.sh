...
#PBS -N akurniawan_pt_mt_adapters_2m
...
TRANSFORMERS_CACHE=$CACHE_FOLDER WANDB_PROJECT=iwslt_adapters_pt_mt_$SAMPLE $PYTHON $WORKDIR/experiments_mt.py \
    ...
    --pretrained_mt_path /storage/brno3-cerit/home/akurniawan/adapters-project/outputs/baseline_normal_2000000/checkpoint-966000 \
    --dataset_loader_script $WORKDIR/dataset/iwslt14/iwslt_loader.py \
    --dataset_dir $WORKDIR/dataset/iwslt14 \
    --output_dir $MODELDIR/outputs/adapters_de_en_pt_mt_$SAMPLE \
