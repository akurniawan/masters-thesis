...
#PBS -N akurniawan_na_tl_adapters_500k
...
TRANSFORMERS_CACHE=$CACHE_FOLDER WANDB_PROJECT=iwslt_adapters_na_tl_$SAMPLE $PYTHON $WORKDIR/experiments_mt.py \
    --enc_config_name bert-base-german-dbmdz-uncased \
    --dec_config_name bert-base-uncased \
    --pretrained_mt_path /storage/brno3-cerit/home/akurniawan/adapters-project/outputs/baseline_normal_500000/checkpoint-498000 \
    --dataset_loader_script $WORKDIR/dataset/iwslt14/iwslt_loader.py \
    --dataset_dir $WORKDIR/dataset/iwslt14 \
    --output_dir $MODELDIR/outputs/adapters_de_en_na_tl_$SAMPLE \
